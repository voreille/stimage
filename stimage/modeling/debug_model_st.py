...
sys.path.append('.')
# download model_st.py from Github and load it
...
save_checkpoint_path = './models/' + pre_trained_model.replace(
    "/", "_") + "/" + tissue_type + "_" + start_time + '/'
os.makedirs(save_checkpoint_path, exist_ok=True)
############################################

device = "cuda:0" if torch.cuda.is_available(
) else "cpu"  # If using GPU then use mixed precision training.

vision_model = CLIPVisionModelWithProjection.from_pretrained(
    pre_trained_model, cache_dir=cache_dir)
image_preprocesser = CLIPProcessor.from_pretrained(
    pre_trained_model, cache_dir=cache_dir).image_processor

data_path = f'./{tissue_type}_count_{gene_type}.csv'
...
data_all = image_gene_data(list_image_path, gene_expression_data,
                           image_preprocesser)

data_all_dataloader = DataLoader(data_all, batch_size=BATCH_SIZE,
                                 shuffle=True)  #Define your own dataloader

# here we use 32 at latent dimension for contrastive learning
st_model = ST_contrastive_model(gene_expression_data.shape[1], 32,
                                vision_model)
...
)  #Params used from paper, the lr is smaller, more safe for fine tuning to new
...
for epoch in range(EPOCH):
    for batch in data_all_dataloader:
        ...
        total_loss = (loss_img(logits_per_image, ground_truth) +
                      loss_txt(logits_per_gene, ground_truth)) / 2
        total_loss.backward()
    ...
    torch.save(
        {
            ...
            'loss': total_loss,
        }, f"{save_checkpoint_path}/model_epoch{epoch}.pt")
