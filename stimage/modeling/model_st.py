...
class image_data(Dataset):
    ...
    def __getitem__(self, idx):
        image = self.image_preprocesser(
            Image.open(self.image_path[idx]),
            return_tensors="pt")['pixel_values'][0]  # Image from PIL module
        return image
...
class image_gene_data(Dataset):
    ...
    def __init__(self, list_image_path, gene_expression, image_preprocesser):
        ...
        if len(self.image_path) != self.gene_expression.shape[0]:
            raise ValueError(
                'The length of image_path and gene_expression should be the same.'
            )
...
class image_gene_data_uni(Dataset):
    ...
    def __getitem__(self, idx):
        image = self.image_preprocesser(Image.open(
            self.image_path[idx]))  # Image from PIL module
        gene_expression = self.gene_expression[idx, :]
        return image, gene_expression
